...
Importers skip previously imported evidence names without comparing contents.
The concurrency importer requires two captured HTTP responses; it does not
import transport-error-only attempts.

Current scripts depend on manually prepared requests and captured evidence.
A clean-clone setup and offline tests remain to be added.
